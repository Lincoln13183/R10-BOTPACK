...
module.exports.run = async function({ api, event, args, getText }) {
 ...
 if (!command) {
  ...
  for (let i = 0; i < visibleCategories.length; i++) {
   ...
   msg += `╭[ ${numberFont[i]} ]─❍ ${category.charAt(0).toUpperCase() + category.slice(1)
    }\n╰┈➤  》${commandNames.join(", ")}\n\n`;
  }
  ...
  msg += `╭ ───────── ╮
│ 𝖯𝖠𝖦𝖤 ${numberFontPage[currentPage - 1]} of ${numberFontPage[totalPages - 1]
   }  │\n╰ ───────── ╯\n`;
  ...
  const imgP = [];
  const img = [
"https://i.imgur.com/ipOvlWn.gif",
"https://i.imgur.com/7ff1e8v.gif",
"https://i.imgur.com/hvcUCC5.gif",
"https://i.imgur.com/fKKULWx.gif",
  ];
